...
def projectData(X,u,K): 
    ...
    u_reduced = u[:,:K]
    projection_K=[]
    for examle in np.arange(len(X)):
        projection=[]
        for eigen_vector in np.arange(K):
            project = X[examle,:].dot(u_reduced[:,eigen_vector])
